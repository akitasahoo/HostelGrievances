<script lang="ts">
	...
	import SecurityAnalyticsDashboard from '$lib/components/app/security-analytics-dashboard.svelte';
	...
	let showSecurityCenter = $state(true);
...
<PageHeader title="Warden dashboard" description="Grievances across all hostel students & cybersecurity command center.">
	{#snippet actions()}
		<Button variant="outline" onclick={() => (showSecurityCenter = !showSecurityCenter)}>
			<ShieldAlertIcon class="size-4 text-emerald-500" />
			{showSecurityCenter ? 'Hide Security Graphs' : 'View Security Graphs'}
		</Button>
