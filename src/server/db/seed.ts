...
export function seedDatabase(db: Database, uploadsDir: string): void {
	...
	const studentPassword = process.env.INITIAL_STUDENT_PASSWORD || 'GIET_Student_2026!';
	const wardenPassword = process.env.INITIAL_WARDEN_PASSWORD || 'GIET_Warden_2026!';

	if (!process.env.INITIAL_STUDENT_PASSWORD || !process.env.INITIAL_WARDEN_PASSWORD) {
		console.warn(
			'[SECURITY WARNING]: Seeding database with default/test credentials. Please define INITIAL_STUDENT_PASSWORD and INITIAL_WARDEN_PASSWORD environment variables for production.'
		);
	}

	const studentHash = hashPassword(studentPassword);
	const wardenHash = hashPassword(wardenPassword);
